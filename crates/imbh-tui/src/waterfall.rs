...
///
/// `name_offset` scrolls the name column horizontally (see [`name_offset`]); every row is shifted by
/// the same amount so the column stays a column, with `<`/`>` markers on the rows that are clipped.
...
pub(crate) fn render_waterfall_row(
    ...
    let indent = (row.indent * 2).min(WATERFALL_NAME_W);
    format!(
        "{}{}{}|{}|{}",
        row.marker,
        " ".repeat(indent),
        clip_field(
            ...
            name_offset.min(row_name_offset(row))
        ),
        bar,
        row.suffix
    )
...
fn row_name_offset(row: &WaterfallRow) -> usize {
    let window = WATERFALL_NAME_W.saturating_sub(row.indent * 2);
...
pub(crate) fn name_offset(rows: &[WaterfallRow], cursor: usize) -> usize {
    rows.get(cursor).map_or(0, row_name_offset)
}
...
mod tests {
    ...
    fn waterfall_bars_align_regardless_of_depth_or_wide_names() {
        ...
        for cells in [40usize, 77] {
            for offset in [0usize, 1, 5, 9] {
                let lines = render_waterfall(&waterfall, cells, offset);
    ...
    fn name_offset_follows_the_cursor_row() {
        ...
        assert_eq!(name_offset(&rows, 0), 0);
        ...
        assert_eq!(name_offset(&rows, 99), 0);
    }
    ...
    fn the_indent_never_scrolls_away_under_a_long_name() {
        ...
        let offset = name_offset(&waterfall.rows, 4);
        assert!(offset > 0);
        let lines = render_waterfall(&waterfall, 20, offset);
