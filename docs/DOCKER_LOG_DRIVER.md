...
```sh
docker plugin install --alias imbh ghcr.io/moriyoshi/imbh-log-driver:0.9.0-amd64
```
...
- **Pick the tag by the VM's architecture, not your machine's marketing name.** Apple silicon runs an
  arm64 VM, so it is `…-log-driver:0.9.0-arm64`. The `-amd64` tag in the recipe above is the Linux
