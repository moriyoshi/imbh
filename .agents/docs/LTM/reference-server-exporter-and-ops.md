...
- Routes: `POST /v1/logs`·`/v1/traces`·`/v1/metrics` (OTLP/HTTP protobuf, gzip accepted; the response is the OTLP one — an `Export<signal>ServiceResponse`, empty on full success, `partial_success` on a rejection, a `google.rpc.Status` on 4xx/5xx, all in the request's declared encoding — with the receipt counts moved to `x-imbh-accepted`/`-rejected`/`-durable`/`-queued` headers, `src/otlp.rs`), `POST /api/query` (body: raw SQL, or a `{"query": "…"}` / bare-JSON-string document when `Content-Type` names JSON — the header alone decides, nothing is sniffed; → JSON rows), `GET /health`, `GET /stats`, `POST /admin/flush`, `POST /admin/compact`. The table is a plain `axum::Router`, public as `imbh_server::app(db)` so a host can mount it in its own application.
...
- `otlp_ingest_answers_the_spec_response` + `a_rejected_point_comes_back_as_partial_success` (lib) and the wire halves in `http_e2e.rs` — the OTLP response contract: full success = zero-byte `Export<signal>ServiceResponse` under the request's `Content-Type` (protobuf when the request declares none, since that is the only payload decoded), rejection = `partial_success.rejected_*` with the shared `REJECTED_MESSAGE` both transports use, failure = `google.rpc.Status` (code 3/5/13 matching `grpc.rs`), receipt counts = `x-imbh-*` headers. `otlp.rs`'s own unit tests pin the wire shapes (empty message, JSON mapping quoting `int64` counts, `Status` field tags).
...
- `query_body_shape_follows_content_type` + `json_content_types_are_recognised` — `POST /api/query` reads its body as raw SQL or as a JSON document strictly by `Content-Type`: no header / `text/plain` / `application/x-www-form-urlencoded` (what `curl --data` sends) stay raw, `application/json` (with parameters, and `+json` suffixes) takes `{"query": …}` or a bare string and must answer *byte-identical* rows to the raw form; `query` is the only accepted field name, so `{"sql": …}` is a `400` like any other spelling, and a JSON document with no query is a `400` naming `"query"`, and a JSON body sent without a JSON content type is still read as SQL. The wire half is in `http_e2e.rs`'s `http_wire_ingest_query_stats_and_errors`.
