...
- Routes: `POST /v1/logs`·`/v1/traces`·`/v1/metrics` (OTLP/HTTP protobuf, gzip accepted), `POST /api/query` (body: raw SQL, or a `{"query": "…"}` / bare-JSON-string document when `Content-Type` names JSON — the header alone decides, nothing is sniffed; → JSON rows), `GET /health`, `GET /stats`, `POST /admin/flush`, `POST /admin/compact`. The table is a plain `axum::Router`, public as `imbh_server::app(db)` so a host can mount it in its own application.
