...
Identify semantically distinct topics. A "topic" is a cohesive area of knowledge that an agent would want to look up as a unit. Examples of good topic boundaries for imbh:

- A specific subsystem or crate (e.g., "WAL and recovery", "Parquet segment lifecycle", "Tantivy↔Parquet RowSelection bridge", "DataFusion providers and typed plans", "OTLP normalization")
- A cross-cutting concern (e.g., "footprint budgets and dependency trimming", "canonical JSON attribute storage", "metric temporality handling")
...
- Filename (kebab-case, descriptive, e.g., `wal-and-recovery.md`)
...
<List of relevant source files/crates with brief descriptions>
...
- **Merge, don't copy-paste.** If multiple journal entries cover the same topic, synthesize them into a coherent narrative. Remove chronological framing ("On 2026-07-18 we discovered...") and write in a timeless reference style.
...
| Document | Summary |
|----------|---------|
| [wal-and-recovery.md](wal-and-recovery.md) | How imbh's WAL, LSN watermark, and crash recovery work |
| ... | ... |
```
